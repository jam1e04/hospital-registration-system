...
import type {HospitalDetail,DepartmentResonseData,LoginData,UserLoginResponseData,WXLoginResponseData, HospitalWorkData,DoctorResponseData,UserResponseData,DoctorInfoData} from './type'
// ts枚举
enum API {
...
// 封装获取科室接口
export const reqHospitalDepartment = (hoscode:string) => request.get<any,DepartmentResonseData>(API.HOSPITALDEPARTMENT_URL+hoscode)
// 获取验证码接口
...
// 获取医生排班数据
export const reqHospitalDocdor = (hoscode:string,depcode:string,workDate:string) =>request.get<any,DoctorResponseData>(API.HOSPITALDOCTOR_URL+`${hoscode}/${depcode}/${workDate}`)
// 获取就诊人信息
